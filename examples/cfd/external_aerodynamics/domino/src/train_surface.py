...
from surface_utils import ScalingFactors, get_keys_to_read_surface_only, coordinate_distributed_environment_surface_only
...
from surface_utils import get_num_vars_surface_only, load_scaling_factors_surface_only, compute_l2_surface_only, all_reduce_dict
...
def validation_step(
    ...
    with torch.no_grad():
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            local_metrics = compute_l2_surface_only(
...
def main(cfg: DictConfig) -> None:
    ...
    domain_mesh, data_mesh, placements = coordinate_distributed_environment_surface_only(cfg)
    ...
    ######################################################
    surf_factors = load_scaling_factors_surface_only(cfg)
    vol_factors = None
    ######################################################
    # Configure the model
    ######################################################
    model_type = cfg.model.model_type
    num_surf_vars, num_global_features = get_num_vars_surface_only(cfg)
    ...
    keys_to_read, keys_to_read_if_available = get_keys_to_read_surface_only(
        cfg, get_ground_truth=True
    )
