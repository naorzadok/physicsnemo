...
Investigation of an alternative to the offline `.bin` graph pipeline: raw Zarr
storage + GPU-side graph construction, for the `xaeronet/surface` workflow. The
dynamic pipeline is added *alongside* the offline one (new `*_dynamic.py` scripts
+ `vtk_to_zarr.py`), so the existing recipe keeps working unchanged.
...
| Train over partitions | [train.py](train.py) | per step |
...
5. **DDP + variable partitions.** `train_dynamic.py` uses `static_graph=True` and
...
per training step (per rank):
  ...
  GPU (side stream, overlaps prev backward):     DynamicGraphBuilder.__call__
        ...
        ▼
  main stream:  MeshGraphNet forward/backward  (train_dynamic.py loop)
```

Component responsibilities:

- **Offline converter (new `vtk_to_zarr.py`)** — sample a single *dense* cloud,
  interpolate fields, write Zarr. No kNN/METIS/`torch.save`; the offline
  `preprocessor.py` is left untouched.
- **`RawZarrSurfaceDataset` (new `dataloader_dynamic.py`)** — return raw per-point
...
- **`train_dynamic.py`** — a copy of the offline training loop with one call to
  the builder per step; the existing `for part in graph_partitions:` loop is
  otherwise unchanged.
...
2. Add the Zarr writer as a new converter (`vtk_to_zarr.py`); the offline
   `preprocessor.py` `.bin` path stays the default until parity is proven.
3. Run training through `train_dynamic.py` (the builder path); benchmark step
   time and storage vs. the baseline before making it the default.
