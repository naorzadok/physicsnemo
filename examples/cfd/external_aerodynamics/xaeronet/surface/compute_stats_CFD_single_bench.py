...
def process_file(bin_file):
    ...
    edge_fields = ["x"]
    global_fields = ["Mach", "ReL", "AOA"] # Add your globals here
    ...
    for field in node_fields + edge_fields + global_fields:
        field_means[field] = 0
        field_square_means[field] = 0
        counts[field] = 0

    # --- 1. Process Global Data (Once per file/graph) ---
    # We take the data from the first partition since it's the same for all partitions
    if graphs.num_parts == 1:
        first_graph = graphs.partitions
    else:
        first_graph = graphs[0]
    for field in global_fields:
        if hasattr(first_graph, field):
            data = getattr(first_graph, field).numpy()
            # Ensure it's a 1D array for consistency
            if data.ndim == 0: data = data.reshape(1)
            
            field_means[field] = np.mean(data)
            field_square_means[field] = np.mean(data**2)
            counts[field] = 1 # We count this as 1 "airfoil" sample
        else:
            print(f"Warning: Global field '{field}' not found in {bin_file}")
            
    # Loop through each partition in the file
    if graphs.num_parts == 1:
        graph = graphs.partitions
        # Process node data
...
def aggregate_results(results):
    ...
    for field in total_mean:
        ...
        global_std[field] = np.where(std < 1e-8, 1e-6, std)  # Avoid division by zero
