...
class PartitionedGraph:
    ...
    def __init__(self, graph: pyg.data.Data, num_parts: int, halo_size: int):
        ...
        if num_parts <= 1:
            self.partitions = []
            # --- BYPASS METIS FOR SINGLE PARTITION ---
            # We create a copy of the graph to act as the single partition
            partition = pyg.data.Data(
                edge_index=graph.edge_index,
                edge_attr=graph.edge_attr,
                num_nodes=graph.num_nodes,
                # For consistency with the rest of your code, we add these mappings
                part_node=torch.arange(graph.num_nodes),
                inner_node=torch.arange(graph.num_nodes),
            )
            # Copy all other attributes (coordinates, pressure, Mach, etc.)
            for k, v in graph.items():
                if k not in ['edge_index', 'edge_attr']:
                    setattr(partition, k, v)

            self.partitions.append(partition)
            print('Done bypassing METIS for single partition graph.')
        else:
            ...
            for i in range(self.num_parts):
                ...
                for k, v in graph.items():
                    ...
                    elif k not in ['edge_index', 'edge_attr']:          
                        # This captures GLOBAL parameters (Mach, ReL, AOA, run_id)
                        # We copy them as-is to every partition
                        setattr(partition, k, v)
                self.partitions.append(partition)
...
class GraphDataset(Dataset):
    ...
    def __init__(self, file_list, mean, std):
        ...
        # Inside GraphDataset.__init__
        self.global_fields = ["Mach", "ReL", "AOA"]
    ...
    def __getitem__(self, idx: int) -> tuple[PartitionedGraph, str]:
        ...
        for part in graphs:
            for key in self.global_fields:
                ...
                normed_val = (val - getattr(self, f"{key}_mean")) / (getattr(self, f"{key}_std") + 1e-8)
                setattr(part, key, normed_val)
