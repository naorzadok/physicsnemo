...
# Per-point node fields carried through the pipeline, in the order train_dynamic.py
...
def single_graph(
    ...
    partitioned path in ``train_dynamic``: ``inner_node`` and ``part_node`` are the
...
def _k_hop_subgraph(
    ...
    inner_mapping : LongTensor [n_seed]
        ...
        ``inner_node`` tensor consumed by train_dynamic.py).
...
def spatial_partition(
    ...
    with the offline PartitionedGraph consumed by train_dynamic.py.
    """
