...
``DynamicGraphBuilder`` used during training. For each test store it
  ...
  2. builds the graph identically to training (multi-resolution / importance
     draw + spatial partitioning, per ``conf/config_dynamic.yaml``) using a fixed seed
...
# Make the parent package importable (mirrors inference.py / train_dynamic.py).
...
def node_features(part) -> torch.Tensor:
    ...
    Matches the feature layout used in ``train_dynamic.py``: coordinates,
...
def main(cfg: DictConfig) -> None:
    ...
    # Graph builder configured identically to train_dynamic: scale-invariant
    ...
    out_json = "inference_dynamic_errors.json"
