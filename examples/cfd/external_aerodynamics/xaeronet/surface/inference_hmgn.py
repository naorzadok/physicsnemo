...
def main(cfg: DictConfig) -> None:
    ...
    for test_graph_partitions, test_id in test_dataloader:
        ...
        for part in test_graph_partitions:
            ...
            # Global (flow) conditions [1, num_global_features] modulate the
            # processor via FiLM; passed separately, not per node.
            global_features = (
                part.global_features
                if "global_features" in part
                else torch.zeros((1, num_global_features), device=device)
            )

            with torch.inference_mode():
                with torch.autocast(amp_device, enabled=True, dtype=amp_dtype):
                    pred = model(ndata, part.edge_attr, part, global_features)[
                        part.inner_node
                    ]
                    target = torch.cat(
