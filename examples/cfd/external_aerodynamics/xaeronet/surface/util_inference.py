...
``inference_dynamic.py`` (integrated with ``aero_coefficients.py``) into a
metric report and a couple of diagnostic plots. It is used two ways:

* **Imported** by ``inference_dynamic.py``, which calls :func:`generate_report`
...
* **Standalone**, re-generating the same report and plots from the saved
  artifacts (``inference_dynamic_errors.json`` + ``inference_cloud_*.vtp``)::

    python util_inference.py \
        --errors-json outputs/XAeroNetS/inference_dynamic_errors.json \
...
def coefficient_metrics(
    ...
    coeff_rows : list of dict
        ...
        coefficient name (as written by ``inference_dynamic.py``).
...
def main() -> None:
    ...
    parser.add_argument(
        "--errors-json",
        required=True,
        help="Path to inference_dynamic_errors.json.",
    )
    parser.add_argument(
        "--clouds-dir",
        default=None,
