...
"""
Standalone utility to visualize the geometry-aware sampling weights that
``vtk_to_zarr.py`` bakes into each surface store as ``sampling_weight``.
...
see where points will concentrate without running the full conversion.
...
from vtk_to_zarr import (
    _cell_curvature_weights as compute_cell_curvature_weights,
    _cell_edge_weights as compute_cell_edge_weights,
    _cell_sampling_weights as compute_cell_sampling_weights,
)
